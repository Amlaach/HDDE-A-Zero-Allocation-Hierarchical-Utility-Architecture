...
pub extern "C" fn hdde_engine_destroy(engine_ptr: *mut HDDEngine) {
    if !engine_ptr.is_null() {
        unsafe { let _ = Box::from_raw(engine_ptr); }
    }
}

#[no_mangle]
pub extern "C" fn hdde_engine_spawn(engine_ptr: *mut HDDEngine, x: f32, y: f32, z: f32, hierarchy: u8) -> u32 {
...
#[no_mangle]
pub extern "C" fn hdde_engine_tick(engine_ptr: *mut HDDEngine, events_ptr: *const FFIEvent, events_count: usize) {
...
#[no_mangle]
pub extern "C" fn hdde_engine_get_actions(engine_ptr: *const HDDEngine, out_actions_ptr: *mut FFIAction, max_actions: usize) -> usize {
    if engine_ptr.is_null() || out_actions_ptr.is_null() { return 0; }
    ...
    for (i, &idx) in active_indices.iter().take(count).enumerate() {
        ...
        match action {
            ...
            crate::core::action::ActionKind::Advance(p) => { ffi.action_type = 1; ffi.pos_x = p.x; ffi.pos_y = p.y; ffi.pos_z = p.z; }
            crate::core::action::ActionKind::Engage(id) => { ffi.action_type = 2; ffi.target_id = id.0; }
            crate::core::action::ActionKind::Flee(p) => { ffi.action_type = 3; ffi.pos_x = p.x; ffi.pos_y = p.y; ffi.pos_z = p.z; }
            _ => ffi.action_type = 0,
        }
