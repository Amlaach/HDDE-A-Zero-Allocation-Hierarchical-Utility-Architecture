...
pub fn run(registry: &mut SoARegistry) {
    ...
    for idx in active_indices {
        ...
        let mut c_idx = 0;
        let mut candidates = [None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None];
        
        macro_rules! add {
            ($kind:expr) => {
                if c_idx < 16 {
                    candidates[c_idx] = Some(ActionCandidate { kind: $kind, score: 0.0 });
                    c_idx += 1;
                }
            }
        }
